...
######################################
if [ $# == 0 ]; then
    echo './volcano.sh namevolcano gacos npara hcorr'
...
gpu="n"	# y/n
check_only="n" # y/n. If y, not run scribatch_LiCSBAS.shpts and just show commands to be done
...
clipinfo=`grep $VOLC_NAME $filevolcano | awk '{print $4}'`
maxheigh=`grep $VOLC_NAME $filevolcano | awk '{print $5}'`
#HMIN=`grep $VOLC_NAME $filevolcano | awk '{print $4}'`

#maxheigh=$(awk '{print $1}' *.txt | sort -nr | head -1)
#minheigh=$(awk '{print $2}' *.txt | sort -nr | head -1)
#clipinfo=$(awk '{print $3}' *.txt | sort -nr | head -1)
...
p120_use="y"  # y/n
p120_sbovl="n"
p12_loop_thre="5"	# default: 1.5 rad. With --nullify, recommended higher value (as this is an average over the whole scene)
...
p13_nullify_noloops="y" # y/n. n by default
p13_singular="" # y/n. n by default
p13_singular_gauss="y" # y/n. n by default
p13_skippngs="y" # y/n. n by default
p13_sbovl="n"
p13_badnoloops="n" #For add bad IFS the noloops
p13_keep_incfile="y" # n by Default --keep_incfile#
p15_coh_thre="0.1"	# default: 0.05
p15_n_unw_r_thre=""	# default: 1.5
p15_vstd_thre=""	# default: 100 mm/yr
p15_maxTlen_thre=""	# default: 1 yr
p15_n_gap_thre="1"	# default: 10
p15_stc_thre=""	# default: 10 mm
p15_n_ifg_noloop_thre="300"	# default: 500 - setting this much higher than orig since we nullify them (p13_nullify_noloops)
p15_n_loop_err_thre=""	# default: 5
p15_n_loop_err_ratio_thre="0.3"	# default: 0.7 - in future we will switch to this ratio term, instead of n_loop_err
...
p04_outGEOCmldir_suffix="_mask" # default: mask
p04_n_para=$n_para   # default: # of usable CPU
p05_inGEOCmldir=""      # default: $GEOCmldir
p05_outGEOCmldir_suffix="_$VOLC_NAME" # default: clip
