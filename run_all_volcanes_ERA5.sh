...
while read -r line; do
    ...
    tmp_script="jasmin_run_ERA5_${clip_name}.sh"
    echo -e "#!/bin/bash\nmodule load licsbas_comet_dev\n./batch_LiCSBAS_ERA5.sh $clip_name" > $tmp_script
    chmod +x $tmp_script

    # Enviar el trabajo a la cola usando bsub2slurm.sh
    parent_dir=$(basename "$(dirname "$(pwd)")")
    current_dir=$(basename "$(pwd)")
    bsub2slurm.sh -o LB_ERA5_${current_dir}_${clip_name}.out \
                  -e LB_ERA5_${current_dir}_${clip_name}.err \
                  -J LiCSBAS_ERA5_${current_dir}_${clip_name} \
